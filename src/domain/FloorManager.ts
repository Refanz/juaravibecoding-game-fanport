...
export class FloorManager {
  ...
  init(): void {
    ...
    this.allObjects = [
      ...
      ...FLOOR3_OBJECTS.map((d) => createInteractable(d, idx++)),
    ];
    this.loadFloor(1);
  }
  ...
  get solvedCount() {
    return this.allObjects.filter((o) => o.solved).length;
  }
  get allSolved() {
    return this.solvedCount >= this.totalObjects;
  }

  get elevatorCenterX() {
    return this.elevatorPos.x * TILE + TILE / 2;
  }
  get elevatorCenterY() {
    return this.elevatorPos.y * TILE + TILE / 2;
  }

  isNearElevator(px: number, py: number): boolean {
    return (
      Math.hypot(px - this.elevatorCenterX, py - this.elevatorCenterY) <
      INTERACT_RANGE
    );
  }
  ...
  get cctvMonitorCenterY() {
    return this.cctvMonitorPos.y * TILE + TILE / 2;
  }

  isNearCCTVMonitor(px: number, py: number): boolean {
    if (this.currentFloor !== 1) return false;
    return (
      Math.hypot(px - this.cctvMonitorCenterX, py - this.cctvMonitorCenterY) <
      INTERACT_RANGE
    );
  }
