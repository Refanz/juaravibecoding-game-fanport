...
const CCTV_FEEDS: CCTVFeed[] = [
  ...
  { id: "server_room", label: "Server Room", floor: "Lt. 3", activity: "🖥️" },
];
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        ctx.beginPath();
        ctx.arc(
          ((Math.sin(t * (0.4 + i * 0.1) + i) + 1) / 2) * (W - 8) + 4,
          ((Math.cos(t * (0.3 + i * 0.07) + i * 2) + 1) / 2) * (H - 8) + 4,
          3,
          0,
          Math.PI * 2,
        );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          style={{ color: "#7fb5c8", fontSize: 7, fontFamily: "monospace" }}
        >
          [{feed.floor}] {clock}
        </span>
      </div>
...
export default function CCTVMonitorModal({ onClose }: Props) {
  ...
  const requestCapture = useCallback(() => {
    const frames = captureAllCCTVFrames();
    setFrames(frames);
    setLoading(false);
  }, []);

  useEffect(() => {
    // Immediately load cached frames from sessionStorage
    ...
    // Initialize renderer (loads tileset + decoration sprites)
    let interval: ReturnType<typeof setInterval> | null = null;
    initCCTVRenderer().then(() => {
      // First live capture
      requestCapture();
      // Refresh every 1.5 seconds for "live" effect
      interval = setInterval(requestCapture, 1500);
    });
  ...
  const handleZoomIn = (id: string) =>
    setZoom((p) => ({ ...p, [id]: Math.min((p[id] ?? 1) + 0.5, 3) }));
  const handleZoomOut = (id: string) =>
    setZoom((p) => ({ ...p, [id]: Math.max((p[id] ?? 1) - 0.5, 0.5) }));
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    if (touchStartX === null || !expanded) return;
    const diff = touchStartX - e.clientX;
    if (Math.abs(diff) > 50) {
      const currentIndex = CCTV_FEEDS.findIndex((f) => f.id === expanded);
      if (diff > 0) {
        // Swipe left -> Next CCTV
        const nextIdx = (currentIndex + 1) % CCTV_FEEDS.length;
        setExpanded(CCTV_FEEDS[nextIdx].id);
      } else {
        // Swipe right -> Previous CCTV
        const prevIdx =
          (currentIndex - 1 + CCTV_FEEDS.length) % CCTV_FEEDS.length;
        setExpanded(CCTV_FEEDS[prevIdx].id);
      }
    }
    setTouchStartX(null);
  };
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ color: "#4fc3f7", fontSize: 10 }}>
              CCTV CONTROL ROOM
            </div>
            <div style={{ color: "#5d8aa0", fontSize: 7, marginTop: 3 }}>
              {loading
                ? "⏳ Memuat live view..."
                : `✅ ${Object.keys(frames).length} kamera aktif`}
            </div>
          </div>
        </div>
      ...
      {expanded && expandedFeed ? (
        ...
        >
          ...
          <CCTVPanel
            feed={expandedFeed}
            snapshot={frames[expandedFeed.id] ?? null}
            expanded
            zoom={zoom[expandedFeed.id] ?? 1}
          ...
          >
            <span>👈 Geser untuk pindah 👉</span>
            <span>[ESC] Tutup · 🔍+/− Zoom</span>
          </div>
        </div>
      ) : (
        <div
      ...
      >
        <span style={{ color: "#2e6a8a", fontSize: 7 }}>
          [ESC] tutup · 🔍+/− zoom · LIVE 1s (TimeManager)
        </span>
